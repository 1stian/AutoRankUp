package no.ohnee.autorankup.config;
...
public class Config {
    ...
    private static void error(Exception e){
        if (AutoRankUp.DEBUG){
            AutoRankUp.log.log(Level.SEVERE, "Something went wrong: {0} ", e);
        }
    }
    ...
    public static boolean getBoolean(String location){
        try {return AutoRankUp.getAutoRankUp().getConfig().getBoolean(location);}
        catch (Exception e) {
            error(e);
            return false;
        }
    ...
    public static int getInteger(String location){
        try {return AutoRankUp.getAutoRankUp().getConfig().getInt(location);}
        catch (Exception e) {
            error(e);
            return 0;
        }
    ...
    public static String getString(String location) {
        try {return AutoRankUp.getAutoRankUp().getConfig().getString(location);}
        catch (Exception e) {
            error(e);
            return "";
        }
    ...
    public static double getDouble(String location) {
        try {return AutoRankUp.getAutoRankUp().getConfig().getDouble(location);}
        catch (Exception e) {
            error(e);
            return 0.0;
        }
    ...
    public static List<String> getList(String location){
        try {return AutoRankUp.getAutoRankUp().getConfig().getStringList(location);}
        catch (Exception e) {
            error(e);
    ...
    public static ConfigurationSection getSection(String section){
        try {return AutoRankUp.getAutoRankUp().getConfig().getConfigurationSection(section);}
        catch (Exception e) {
            error(e);
            return null;
        }
    ...
    public static boolean reloadConfig(){
        try {
            AutoRankUp.getAutoRankUp().reloadConfig();
            return true;
        }catch (Exception e) {
            error(e);
            return false;
