package no.ohnee.autorankup.checks;

import no.ohnee.autorankup.AutoRankUp;
import no.ohnee.autorankup.bungee.Messages;
import no.ohnee.autorankup.config.Config;
import no.ohnee.autorankup.helpers.Enums;
...
import static no.ohnee.autorankup.AutoRankUp.*;
...
public class CanRankUp {
    ...
    public static void rankUp(Player player){
        ...
        if (rankChecker(player, playerGroup)){
            ...
            log.log(Level.INFO, "[AutoRankUp] - No players eligible.");
        }
    }
    ...
    private static void sendMessages(Player player, String toRank){
        if (getPlayerEnb()){
            String rPlayer = AutoRankUp.getPlayerMsg().replace(playerTag, player.getName());
        ...
        if (getBroadcastEnabled() && !getBungeeBroadcast()){
            String rPlayer = AutoRankUp.getBroadcastMsg().replace(playerTag, player.getName());
            String rGroup = rPlayer.replace(groupToTag, toRank);
